...
METRICAS = [
    ...
    "icir", "mean_n_assets", "n_periods",
]
QUANTIS = [0.01, 0.05, 0.25, 0.50, 0.75, 0.95, 0.99]
...
def _extract_params(payload: dict[str, Any]) -> dict[str, Any]:
    return _first_dict(payload.get("params"), payload.get("config"), payload.get("args"), payload.get("hyperparameters"))
...
def _tipo_dado(text: str, params: dict[str, Any]) -> str | float:
    ...
    for value in candidates:
        ...
        if "price" in low or "preco" in low or "preço" in low or "b3_daily_tfb" in low:
            return "preços"
    return np.nan
...
def _metadata(path: Path, root: Path, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if pd.isna(pred_len):
        pred_len = _find_int(text, [r"pred[_-]?len[_-]?(\d+)", r"horizon[_-]?(\d+)", r"h[_-]?(\d+)", r"pred[_-]?(\d+)"])
    janela = _param_int(params, ["janela_trading", "rebalance_k", "trading_window", "k", "window"])
    if pd.isna(janela):
        m = re.findall(r"(?:^|[/_-])k[_-]?(\d+)(?:$|[/_-])", text, flags=re.IGNORECASE)
        janela = int(m[-1]) if m else pred_len
    top_j = _param_int(params, ["top_j", "topj", "max_assets", "j", "top"])
    if pd.isna(top_j):
        top_j = _find_int(text, [r"top[_-]?j[_-]?(\d+)", r"top[_-]?(\d+)", r"j[_-]?(\d+)"])
    return {
        "grupo": "RandomTopJ", "modelo": "RandomTopJ", "tipo_dado": _tipo_dado(text, params),
        "lookback": lookback, "pred_len": pred_len, "janela_trading": janela,
        "top_j": top_j, "run": path.parent.name, "json_path": str(path),
    }
...
def resumir_randomtopj(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, part in df.groupby(group_cols, dropna=False):
        ...
        base["n_simulacoes"] = len(part)
        for metric in metric_cols:
            s = part[metric].dropna()
            base[f"{metric}_n"] = int(len(s))
            if s.empty:
                continue
            base[f"{metric}_mean"] = float(s.mean())
            base[f"{metric}_median"] = float(s.median())
            base[f"{metric}_std"] = float(s.std(ddof=1)) if len(s) > 1 else 0.0
            base[f"{metric}_min"] = float(s.min())
            base[f"{metric}_max"] = float(s.max())
            for q in QUANTIS:
                base[f"{metric}_q{int(q * 100):02d}"] = float(s.quantile(q))
        rows.append(base)
    return pd.DataFrame(rows)


def main() -> None:
    parser = argparse.ArgumentParser(description="Compila JSONs de RandomTopJ em bases longas e resumos empíricos.")
