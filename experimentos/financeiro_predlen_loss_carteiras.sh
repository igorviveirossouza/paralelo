...
#SBATCH --job-name=fin_predlen
#SBATCH --output=/sonic_home/igor.viveiros/paralelo/logs/fin-predlen-%j.out
#SBATCH --error=/sonic_home/igor.viveiros/paralelo/logs/fin-predlen-%j.err

set -euo pipefail

PROJECT_DIR="${PROJECT_DIR:-/sonic_home/igor.viveiros/paralelo}"
VENV_PATH="${VENV_PATH:-/sonic_home/igor.viveiros/py310}"
...
EXPERIMENTO="predlen_loss"
PRED_ROOT="previsoes/financeiro_${EXPERIMENTO}"
RAW_PRED_ROOT="${PRED_ROOT}/_raw"
SIM_ROOT="simulacoes/financeiro_${EXPERIMENTO}"
...
echo "=== Experimento pred_len/loss iniciado em $(date) ==="
  ...
  for MODEL_NAME in "${MODELOS[@]}"; do
    for LOOKBACK in "${LOOKBACKS[@]}"; do
      for PRED_LEN in "${PRED_LENS[@]}"; do
        ...
        echo ""
        echo "------------------------------------------------------------"
        echo "Treinando/previsões | tipo=$TIPO_SERIE | modelo=$MODEL_NAME | lookback=$LOOKBACK | pred_len=$PRED_LEN"
        echo "------------------------------------------------------------"
...
echo ""
echo "=== Experimento pred_len/loss finalizado em $(date) ==="
echo "Previsões: $PRED_ROOT"
echo "Simulações: $SIM_ROOT"
echo "Use pivot_compartivo_metricas.ipynb apontando para: $SIM_ROOT"
